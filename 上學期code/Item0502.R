# movies=read.csv('/Users/StanleyLIn/Desktop/Movieinfo 2.csv') #這邊我有改，你去github把他拿下來，我有更新了就是我的第一列有放標題，這樣讀才會對
 ...
 recommender_model <- Recommender(ratingmat_norm, method = "IBCF", param=list(method="Cosine",k=300)) #取最靠近的前30個neighbors
 recom_results=rep(list(list()),943)
 for(k in 1:943)
 {
 	recom <- predict(recommender_model, ratingmat[k], n=3) #ratingmat[1]就是用user 1去做,10是最close的前10個項目
 	recom_list <- as(recom, "list") 
 	recom_result <- matrix(0,3)
 	for (i in c(1:3))
 	{
...
print(recom_results)
write.csv(recom_results,file="HEY2.csv")
