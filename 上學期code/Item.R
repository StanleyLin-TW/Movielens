...
strC=".test"
 ...
 library(Matrix)
 ratingmats=sparseMatrix(i=ratings[,1], j=ratings[,2],x=ratings[,3])
 library(recommenderlab)
 print(dim(ratingmats))
 ratingmat <-new("realRatingMatrix", data = ratingmats)
 print(dim(ratingmat))
 #ratingmat_norm <- normalize(ratingmat)
 recommender_model <- Recommender(ratingmat, method = "IBCF", param=list(normalize="center",method="Cosine",k=100000 )) #取最靠近的前30個neighbors
...
print(length(test[,3]))
rmses=sqrt(sum((get_value-test[,3])^2,na.rm=TRUE)/length(get_value))
print(rmses)
